class MapPlot {
  makeColorbar(
    ...
    // Axis numbers
    const colorbar_axis = d3
      .axisLeft(value_to_svg)
      .tickFormat(d3.format(".0f"));

    const colorbar_g = this.svg
      .append("g")
      .attr("id", "colorbar")
    ...
    Promise.all([population_promise, map_promise, point_promise]).then(
      (results) => {
        ...
        this.label_container
          ...
          .attr(
            "transform",
            (d) => "translate(" + path_generator.centroid(d) + ")"
          )
